...
const model = {
    ...
    filterCourses: (id, level, price) => {
        const sql = `select courses.course_id,
        course_name,course_image,course_level,
        course_duration, course_price, c.category_name, course_author.user_id, u.first_name, u.last_name, u.user_avatar, count(video_name) as video_count from courses left join course_author on courses.course_id = course_author.course_id left join users as u on course_author.user_id = u.user_id left join categories as c on courses.category_id = c.category_id left join topics as t on courses.course_id = t.course_id left join videos on t.topic_id = videos.topic_id WHERE CASE WHEN 0 < $1 THEN c.category_id = $1 ELSE c.category_id = ANY
        (SELECT category_id
